...
export const cachedFetch = unstable_cache(flagFetch, [FLAG]);

export const cachedFetchAction = async (_: any, data: FormData) => {
  const body = coolerJsonParse(data.get("body")!.toString().trim());

  return cachedFetch(data.get("url")!.toString(), body);
};

/**
 * Reinventing the wheel and make it triangle
...
function coolerJsonParse(str: string, ret: any = {}) {
  if (str[0] !== "{" || str.at(-1) !== "}") return {};

  const matches = str
    .slice(1, str.length - 1)
    .matchAll(/(?:^|,)\s*"(\w+)"\s*:\s*(\d+|"\w+"|\{.+?})/g);
